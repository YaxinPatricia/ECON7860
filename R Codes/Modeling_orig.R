...
for (m in models) {
  ...
  if (m == 'rf') {
    ...
    glmnet_best[['f1_val']] <- F_meas(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
    glmnet_best[['confm']] <- confusionMatrix(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
  } else if (m == 'knn') {
    m1 <- train(left ~ ., data =  cbind(scale(X_dummy_train[ , 1 : 5]), X_dummy_train[ , 6 : length(X_dummy_train)]), method = m,
                metric = 'F', tuneLength = tune, trControl = control, tuneGrid = expand.grid(k = c(2, 3, 4, 5, 10)))
    knn_best[['model']] <- m1
    knn_best[['f1_val']] <- F_meas(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
    knn_best[['confm']] <- confusionMatrix(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
  } else {
    m1 <- train(left ~ ., data =  cbind(scale(X_dummy_train[ , 1 : 5]), X_dummy_train[ , 6 : length(X_dummy_train)]), method = m,
                metric = 'F', tuneLength = tune, trControl = control)
    svmLinear_best[['model']] <- m1
    svmLinear_best[['f1_val']] <- F_meas(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
    svmLinear_best[['confm']] <- confusionMatrix(predict(m1, cbind(scale(X_dummy_test[ , 1 : 5]), X_dummy_test[ , 6 : length(X_dummy_test)])), y_test)
  }
...
#save.image("D:/Yaxin/HKBU BM/Courses/Sem 2/ECON7860 Big Data Analytics for Business (S11)/Group Project/HR Analytics/working/final1.RData")
